          ...
          if (!DRY) {
            ...
            if (out.status === "confirmed" || out.status === "matched") {
              ...
              await db.transaction(async (tx) => {
                ...
                await tx.insert(s.auditLogs).values({ actorRole: "system", action: out.status === "confirmed" ? "registration.confirmed" : "registration.matched", entityType: "doctor", entityId: d.id, after: { council: m.council, number: m.registrationNo, year: m.year, degree: m.detail?.degree ?? null, university: m.detail?.university ?? null, source: "nmc-imr", query: out.query }, reason: "NMC Indian Medical Register match (scripts/enrich.ts)" });
